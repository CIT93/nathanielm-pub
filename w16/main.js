...
function start(first, last, houseHoldMembers, houseSize) {
  // Define a function called start that takes four parameters: first name, last name, number of household members, and house size.
  ...
  cfpData.push({
    // Add a new object to the cfpData array containing details about the individual's carbon footprint.
...
const validateField = (event) => {
  // Define a function called validateField that takes an event parameter.
...
FORM.addEventListener("submit", (e) => {
  ...
  if (FNAME.value !== "" && LNAME.value !== "") {
    ...
    const fpObj = new FP(
      FNAME.value,
      LNAME.value,
      parseInt(e.target.housem.value, 10),
      e.target.houses.value,
      e.target.food.value,
      e.target.foodSource.value,
      e.target.dishwasherUsage.value,
      e.target.washingMachineUsage.value,
      e.target.purchases.value
    );
    cfpData.push(fpObj);
    saveLS(cfpData);
    renderTbl(cfpData);
